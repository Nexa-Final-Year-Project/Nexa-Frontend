...
export default function ProfileDropdown({ user }: { user: User | null }) {
  ...
  if (!user) {
    return (
      ...
      >
        <UserIcon className={`w-5 h-5 ${isDark ? "text-white/60" : "text-neutral-600"}`} />
      </div>
    );
  }
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <div className="relative group cursor-pointer">
          <Avatar className={`h-10 w-10 rounded-xl border-2 transition-all duration-300 group-hover:shadow-lg ${
            isDark 
      ...
      >
        ...
        <DropdownMenuLabel className="p-0">
          <div className={`relative p-4 ${isDark ? "bg-gradient-to-br from-white/[0.04] to-transparent" : "bg-gradient-to-br from-neutral-50 to-transparent"}`}>
            {/* Decorative gradient */}
            ...
            <div className="relative flex items-center gap-3">
              <Avatar className={`h-12 w-12 rounded-xl shadow-lg ${isDark ? "border-2 border-white/[0.1]" : "border-2 border-neutral-200"}`}>
                <AvatarImage
                  src={user.photoURL}
                  alt={user.name}
                  className="rounded-xl"
