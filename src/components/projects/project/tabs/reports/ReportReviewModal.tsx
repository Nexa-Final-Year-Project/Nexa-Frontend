...
const getMongoId = (report: any): string | null => {
  ...
  if (typeof report._id === 'string' && report._id.length === 24) return report._id;
  // MongoDB Extended JSON format { $oid: "..." }
  if (report._id && typeof report._id === 'object' && report._id.$oid) return report._id.$oid;
  // Maybe stored as id instead of _id
  if (typeof report.id === 'string' && report.id.length === 24) return report.id;
  if (report.id && typeof report.id === 'object' && report.id.$oid) return report.id.$oid;
  // Backend returns reportId as the MongoDB _id
  if (typeof report.reportId === 'string' && report.reportId.length === 24) return report.reportId;
  if (report.reportId && typeof report.reportId === 'object' && report.reportId.$oid) return report.reportId.$oid;
  return null;
};
...
export default function ReportReviewModal({
  ...
  const backlogSummary = meta?.backlogSummary || 
    (summary?.backlogCompleteness ? `Generated ${summary.totalTasks || 0} tasks across ${summary.totalPhases || 0} phases. Backlog completeness: ${summary.backlogCompleteness}%. The focus is heavily weighted toward ${Object.keys(report?.workloadDifficulty || {}).find(k => (report?.workloadDifficulty?.[k] || 0) > 0) || 'development'}.` : null) ||
    `Generated ${summary.totalTasks || '?'} tasks across ${summary.totalPhases || '?'} phases.`;
  
  // Get sprint health score from summary or meta
  const sprintHealthScore = summary?.sprintHealthScore ?? meta?.sprintHealthScore ?? null;
  
  // Get member capacity from report root or meta
  const reportMemberCapacity = report?.memberCapacity || meta?.memberCapacity || {};
  
  // Get workload difficulty from report root or meta
  const reportWorkloadDifficulty = report?.workloadDifficulty || meta?.workloadDifficulty || {};
  ...
  const rawAssignmentReasons = report?.rawAgentOutput?.meta?.assignmentReasons || 
    report?.rawAgentOutput?.assignmentReasons || 
    meta?.assignmentReasons || [];
  const [assignmentReasons, setAssignmentReasons] = useState<any[]>(rawAssignmentReasons);
  
  const [showAssignmentModal, setShowAssignmentModal] = useState(false);
  const [memberCapacity, setMemberCapacity] = useState<Record<string, any>>(
    reportMemberCapacity
  );
  ...
  const saveDraft = async () => {
    if (!canEdit) {
      toast.error("You cannot save this report — it belongs to another account");
      return;
    }
    ...
    try {
      ...
      if (mongoId) {
        await reportsApi.updateReport(mongoId, {
          ...
          decisionLog,
          memberCapacity,
          workloadDifficulty: report.meta?.workloadDifficulty || report.workloadDifficulty,
          reliabilityHistory: report.meta?.reliabilityHistory || report.reliabilityHistory,
        });
  ...
  const approveAndAssign = async () => {
    if (!canEdit) {
      toast.error("You cannot approve this report — it belongs to another account");
      return;
    }

    // Extract MongoDB ID from report
    const mongoId = getMongoId(report);
    console.log("approveAndAssign: extracted mongoId:", mongoId, "from report:", { _id: report?._id, id: report?.id, reportId: report?.reportId });

    if (!mongoId) {
      toast.error("Report not synced to database. Cannot approve.");
      console.error("approveAndAssign: could not extract MongoDB ID from report", report);
      return;
    }
    ...
    try {
      ...
      try {
        ...
        console.warn("approveAndAssign: save failed (continuing with approval):", saveErr?.message);
      }
  ...
  const rejectReport = async () => {
    if (!canEdit) {
      toast.error("You cannot reject this report — it belongs to another account");
      return;
    }
      ...
      {open && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
          ...
          <motion.div 
            ...
            animate="visible"
            exit="exit"
            className="relative w-[92vw] max-w-6xl max-h-[90vh] bg-neutral-900/95 rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-white/[0.08]"
          >
            {/* Decorative gradient */}
            ...
            <motion.div 
              ...
              className="flex items-center justify-between px-6 py-4 bg-neutral-900/90 border-b border-white/[0.06] text-white z-10"
            >
              <div>
                <motion.div 
                  initial={{ opacity: 0 }}
              ...
              >
                <Input
                  ...
                  className="px-3 py-2 rounded-xl bg-white/[0.04] text-white placeholder:text-white/30 border border-white/[0.08] focus:border-violet-500/50 transition-all"
                />
...
{isApproved && !isEditing ? (
            ...
            >
              <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                ...
                >
                  <h3 className="font-semibold text-white/70 text-sm">Sprint Health Score</h3>
                  <motion.div 
              ...
              <motion.div variants={itemVariants} className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                >
                  <h4 className="font-medium mb-4 text-white/80">Task Distribution by Role</h4>
                  {Object.entries(taskDistribution).length === 0 && (
                    <div className="text-sm text-white/40 italic">No data</div>
                  )}
                  {Object.entries(taskDistribution).map(([role, count], idx) => (
                    <motion.div 
                      key={role} 
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                ...
                <motion.div 
                  ...
                  className="p-5 rounded-2xl bg-white/[0.02] border border-white/[0.06] text-white hover:border-white/[0.1] transition-all"
                >
                  <h4 className="font-medium mb-4 text-white/80">Workload Difficulty by Role</h4>
                  {Object.entries(workload).map(([role, value], idx) => (
                    ...
                    >
                      ...
                      <div className="w-8 text-sm text-white/50 text-right">{String(value as number)}</div>
                    </motion.div>
                  ))}
                </motion.div>
              </motion.div>
              ...
              <motion.div variants={itemVariants} className="space-y-6">
                <section>
                  ...
                  <p className="text-xs text-white/40 mb-3">AI reasoning and decisions during task generation (read-only)</p>
                  <div className="rounded-xl max-h-48 overflow-y-auto bg-white/[0.02] border border-white/[0.06] p-4">
                    {decisionLog.length === 0 ? (
                      <div className="text-sm text-white/40 italic">No decision log entries available.</div>
                    ) : (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="text-sm text-white/60">
                        <span className="text-white font-medium">{(assignmentReasons || []).length}</span> suggestion(s)
                      </div>
                      <motion.div whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
                        <Button
                          onClick={() => setShowAssignmentModal(true)}
                    ...
                    <p className="text-xs text-white/40 mt-3">
                      Assignment suggestions are reviewed in a dedicated modal where you can edit, approve, or delete individual suggestions.
                    </p>
                  </motion.div>
                </section>
                  ...
                  <div className="rounded-xl max-h-60 overflow-auto bg-white/[0.02] border border-white/[0.06]">
                    <table className="w-full text-sm text-white">
                      <thead className="bg-white/[0.04] text-white/60 sticky top-0">
                        <tr>
                          <th className="text-left px-4 py-3 font-medium">Member Name</th>
                          <th className="text-left px-4 py-3 font-medium">Total Capacity</th>
                          <th className="text-left px-4 py-3 font-medium">Assigned Tasks</th>
                          <th className="text-left px-4 py-3 font-medium">Max Tasks</th>
                        </tr>
                      </thead>
                      <tbody>
                        {Object.entries(memberCapacity).map(([name, cap], idx) => (
                          <motion.tr 
                            key={name} 
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                              ...
                              {isEditing ? (
                                <Input
                                  ...
                                  onChange={(e) =>
                                    setMemberCapacity({
                                      ...memberCapacity,
                                      [name]: { ...cap, total: Number(e.target.value) },
                                    })
                                  }
                                />
                              ) : (
                                <span className="text-white/60">{cap.total ?? '—'}</span>
                              )}
                            </td>
                            <td className="px-4 py-3 text-white/60">{cap.assignedTasks ?? '—'}</td>
                            <td className="px-4 py-3">
                              {isEditing ? (
                                <Input
                                  className="w-24 bg-white/[0.04] border border-white/[0.1] rounded-lg px-3 py-1.5 text-sm text-white focus:border-violet-500/50"
                                  ...
                                  onChange={(e) =>
                                    setMemberCapacity({
                                      ...memberCapacity,
                                      [name]: { ...cap, maxTasks: Number(e.target.value) },
                                    })
                                  }
                                />
                              ) : (
                                <span className="text-white/60">{cap.maxTasks ?? '—'}</span>
                              )}
                            </td>
                          </motion.tr>
                        ))}
              ...
              <AssignmentReviewModal
                ...
                onSave={(updated) => {
                  ...
                  try {
                    ...
                    localStorage.setItem("generationReports", JSON.stringify(updatedAll));
                    try {
                      window.dispatchEvent(new Event("generationReports:changed"));
                    } catch (e) {}
                  } catch (e) {}
