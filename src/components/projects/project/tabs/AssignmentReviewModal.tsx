...
const getMongoId = (obj: any): string | null => {
  ...
  if (typeof obj.reportId === 'string' && obj.reportId.length === 24) return obj.reportId;
  if (obj.reportId && typeof obj.reportId === 'object' && obj.reportId.$oid) return obj.reportId.$oid;
  return null;
};
...
export default function AssignmentReviewModal({
  ...
  const initial = (report?.meta?.assignmentReasons as any[]) || 
  ...
  useEffect(() => {
    if (open && mongoId) {
      console.log("AssignmentReviewModal: loading suggestions for mongoId:", mongoId);
      loadSuggestions();
    } else if (open && !mongoId) {
      console.warn("AssignmentReviewModal: mongoId is missing, checking assignmentSuggestions", { 
        report,
        assignmentSuggestions: report?.assignmentSuggestions 
      });
      // Try to use embedded assignmentSuggestions from report if available
      if (report?.assignmentSuggestions && Array.isArray(report.assignmentSuggestions)) {
        const embeddedSuggestions = report.assignmentSuggestions.map((s: any) => ({
          _id: s.suggestionId || getMongoId(s),
          status: 'suggested',
          ...s
        }));
        console.log("AssignmentReviewModal: using embedded suggestions:", embeddedSuggestions.length);
  ...
  React.useEffect(() => {
    // Get assignment reasons from multiple possible locations
    const reasons = (report?.meta?.assignmentReasons as any[]) || 
      (report?.rawAgentOutput?.meta?.assignmentReasons as any[]) || 
      (report?.rawAgentOutput?.assignmentReasons as any[]) || [];
    setItems(reasons);
  ...
  const loadSuggestions = async () => {
    try {
      ...
      if (response.success) {
        ...
        const allApproved = (response.suggestions || []).every((s: any) => s.status === 'approved');
      ...
      if (report?.assignmentSuggestions && Array.isArray(report.assignmentSuggestions)) {
        ...
        console.log("loadSuggestions: using fallback embedded suggestions:", embeddedSuggestions.length);
        setSuggestions(embeddedSuggestions);
      }
    }
  };
  ...
  const filtered = items.filter((it) => {
    ...
    const hay = `${it.task_title || ""} ${it.member_name || ""} ${it.reason || ""}`.toLowerCase();
  ...
  const approveAll = async () => {
    ...
    console.log("approveAll: suggestion details:", suggestions.map(s => ({ id: s._id, status: s.status })));
    ...
    try {
      ...
      console.log("approveAll: pendingSuggestions:", pendingSuggestions.length, "suggestionIds:", suggestionIds);
      ...
      if (suggestionIds.length === 0) {
        ...
        if (report?.assignmentSuggestions && Array.isArray(report.assignmentSuggestions)) {
          ...
          console.log("approveAll: falling back to report.assignmentSuggestions, found:", localIds.length, localIds);
          
          if (localIds.length > 0) {
            const response = await reportsApi.bulkApproveSuggestions(localIds, user?.id);
      ...
      console.log("approveAll: calling bulkApproveSuggestions with:", suggestionIds);
      const response = await reportsApi.bulkApproveSuggestions(suggestionIds, user?.id);
      ...
      toast.error(error?.response?.data?.message || error?.message || "Failed to approve all assignments");
    } finally {
  ...
  const removeItem = async (idx: number) => {
    ...
    try {
      if (suggestion?._id) {
        await reportsApi.declineSuggestion(suggestion._id, user?.id, "Removed by PM");
      }
      ...
      {open && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center">
          ...
          <motion.div 
            ...
            animate="visible"
            exit="exit"
            className="relative w-[90vw] max-w-4xl max-h-[88vh] bg-neutral-900/95 rounded-2xl shadow-2xl overflow-hidden flex flex-col border border-white/[0.08]"
          >
            {/* Header */}
            <div className="sticky top-0 z-20 bg-neutral-900/95 backdrop-blur-xl p-5 border-b border-white/[0.06]">
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/50">Review & confirm task assignment suggestions</p>
                    {isApproved && (
                      <div className="flex items-center gap-1.5 mt-1">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400" />
                        <span className="text-xs text-emerald-400">All assignments approved</span>
                      </div>
                    )}
                  </div>
                </div>
                <motion.button
                  ...
                  whileTap={{ scale: 0.9 }}
                  onClick={onClose}
                  className="w-8 h-8 rounded-lg bg-white/[0.04] hover:bg-white/[0.08] border border-white/[0.06] flex items-center justify-center text-white/50 hover:text-white transition-all"
                >
                  <X className="w-4 h-4" />
                </motion.button>
              </div>
              ...
              <div className="flex items-center gap-3 mt-4">
                <div className="relative flex-1">
                  ...
                  <Input 
                    ...
                    className="w-full pl-10 pr-4 py-2.5 bg-white/[0.03] border border-white/[0.06] rounded-xl text-white placeholder:text-white/30 focus:border-emerald-500/30 focus:ring-1 focus:ring-emerald-500/20" 
                  />
                </div>
                <Button 
                  onClick={approveAll}
                ...
                >
                  ...
                  {isApproving ? "Approving..." : isApproved ? "All Approved" : "Approve All"}
                </Button>
                <Button 
                  variant="outline" 
                  onClick={saveAndClose}
                  disabled={isSaving}
                  className="px-4 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.02] text-white hover:bg-white/[0.04] disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer flex items-center gap-2"
                >
            ...
            <div className="flex-1 overflow-auto p-5">
              {filtered.length === 0 ? (
                ...
                >
                  ...
                  <p className="text-sm text-white/50">No assignment suggestions available</p>
                  <p className="text-xs text-white/30 mt-1">Generate tasks with AI to see assignment suggestions here</p>
                </motion.div>
              ) : (
                      ...
                      <div className="flex items-start justify-between gap-4 mb-4">
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-sm font-medium text-white truncate">{it.task_title || 'Untitled task'}</h3>
                            {it.approved && (
                      ...
                      <div className="space-y-3">
                        <div>
                          ...
                          <Input 
                            ...
                            onChange={(e) => { 
                              const c=[...items]; 
                              c[items.indexOf(it)]={...c[items.indexOf(it)], task_title:e.target.value}; 
                              setItems(c); 
                            }} 
                        ...
                        <div>
                          <label className="text-xs text-white/40 mb-1.5 block">Assigned Member</label>
                          <Input 
                            value={it.member_name || ''} 
                            onChange={(e) => { 
                              const c=[...items]; 
                              c[items.indexOf(it)]={...c[items.indexOf(it)], member_name:e.target.value}; 
                              setItems(c); 
                            }} 
                            className="w-full bg-white/[0.03] border border-white/[0.06] rounded-lg px-3 py-2 text-sm text-white focus:border-emerald-500/30 focus:ring-1 focus:ring-emerald-500/20" 
                          />
                        ...
                        <div>
                          <label className="text-xs text-white/40 mb-1.5 block">Assignment Reason</label>
                          <Textarea 
                            value={it.reason || ''} 
                            onChange={(e) => { 
                              const c=[...items]; 
                              c[items.indexOf(it)]={...c[items.indexOf(it)], reason:e.target.value}; 
                              setItems(c); 
                            }} 
                          ...
                          >
                            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                              <div>
                                ...
                                <p className="text-white/70">{it.priority || '—'}</p>
                              </div>
                              <div>
                                <span className="text-white/30">Role</span>
                                <p className="text-white/70">{it.role || '—'}</p>
                              </div>
                              <div>
                                <span className="text-white/30">Complexity</span>
                                <p className="text-white/70">{it.complexity || '—'}</p>
                              </div>
                              <div>
                                <span className="text-white/30">Type</span>
                                <p className="text-white/70">{it.type || '—'}</p>
                              </div>
                              <div>
                                <span className="text-white/30">Risk Level</span>
                                <p className="text-white/70">{it.riskLevel || '—'}</p>
                              </div>
                              <div>
                                <span className="text-white/30">Estimated Hours</span>
                                <p className="text-white/70">{it.estimatedHours ?? '—'}</p>
                              </div>
                            </div>
                            {it.definitionOfDone && (
                              <div className="mt-3 pt-3 border-t border-white/[0.04]">
                                <span className="text-xs text-white/30">Definition of Done</span>
                                <p className="text-xs text-white/70 mt-1 whitespace-pre-wrap">{it.definitionOfDone}</p>
                              </div>
                            )}
                            {(it.tags?.length > 0 || it.potentialBlockers?.length > 0) && (
                              <div className="mt-3 pt-3 border-t border-white/[0.04] flex flex-wrap gap-4">
                                {it.tags?.length > 0 && (
                                  <div>
                                    <span className="text-xs text-white/30 block mb-1.5">Tags</span>
                                    <div className="flex flex-wrap gap-1">
                                      {it.tags.map((t: string, i: number) => (
                                        <span key={i} className="px-2 py-0.5 text-xs rounded-md bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">{t}</span>
                                      ))}
                                    </div>
                                  </div>
                                )}
                                {it.potentialBlockers?.length > 0 && (
                                  <div>
                                    <span className="text-xs text-white/30 block mb-1.5">Potential Blockers</span>
                                    <div className="flex flex-wrap gap-1">
                                      {it.potentialBlockers.map((b: string, i: number) => (
                                        <span key={i} className="px-2 py-0.5 text-xs rounded-md bg-amber-500/10 text-amber-400 border border-amber-500/20">{b}</span>
                                      ))}
                                    </div>
                                  </div>
                                )}
