...
export const SprintSettingsModal: React.FC<SprintSettingsModalProps> = ({
  ...
  useEffect(() => {
    if (sprint) {
      // Normalize date strings to YYYY-MM-DD so HTML date inputs load correctly
      const normalizeDate = (d?: string) => {
        if (!d) return "";
        try {
          // If it's already in YYYY-MM-DD, format/parseISO will still work
  ...
  // Compute project id (may be empty string) so hook order is stable
  const projectId = sprint
    ? (sprint.project as string) || (sprint.projectId as string) || ""
    : "";
  // Project members for avatars / lookup - call hook unconditionally to preserve hooks order
