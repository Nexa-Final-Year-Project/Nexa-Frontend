  ...
  return (
    <div className="flex flex-col items-center justify-center gap-5">
      <div className={`relative ${sizeMap[size].container}`}>
        {/* Outer rotating ring */}
        <motion.div
          className={`absolute inset-0 ${sizeMap[size].ring} rounded-full border ${isDark ? "border-white/[0.08]" : "border-neutral-200"}`}
          animate={{ rotate: 360 }}
          transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
        >
          <motion.div
            className={`absolute -top-0.5 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full ${isDark ? "bg-white" : "bg-neutral-600"}`}
        ...
        {/* Middle pulsing ring */}
        <motion.div
          className={`absolute inset-2 rounded-full ${isDark ? "bg-white/5" : "bg-neutral-200/50"}`}
          animate={{ 
            scale: [1, 1.1, 1],
            opacity: [0.3, 0.6, 0.3]
          }}
        ...
        >
          <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isDark ? "bg-white/60" : "bg-neutral-500"}`} />
          <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full ${isDark ? "bg-white/40" : "bg-neutral-400"}`} />
        </motion.div>
        ...
        <motion.div
          ...
          transition={{ duration: 1.5, repeat: Infinity }}
        >
          <div className={`w-2 h-2 rounded-full ${isDark ? "bg-white/80" : "bg-neutral-700"}`} />
        </motion.div>
      </div>
      ...
      {text && (
        <motion.p
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className={`text-sm ${isDark ? "text-white/50" : "text-neutral-500"} tracking-wide`}
        >
          {text}
        </motion.p>
      )}
        ...
        >
          ...
          >
            <motion.div 
              ...
              animate={{ boxShadow: ["0 0 8px rgba(34,211,238,0.5)", "0 0 16px rgba(34,211,238,0.8)", "0 0 8px rgba(34,211,238,0.5)"] }}
              transition={{ duration: 1.5, repeat: Infinity }}
            />
          </motion.div>
          ...
          >
            <motion.div 
              ...
              animate={{ 
                scale: [0.8, 1.1, 0.8],
                boxShadow: ["0 0 10px rgba(255,255,255,0.3)", "0 0 25px rgba(255,255,255,0.6)", "0 0 10px rgba(255,255,255,0.3)"]
              }}
              transition={{ duration: 1.5, repeat: Infinity }}
            />
          </motion.div>
...
export const AuthLoading: React.FC<{ text?: string }> = ({ text = "Signing you in..." }) => {
  return (
      ...
      >
        ...
        <div className="relative w-32 h-32">
          ...
          <motion.div
            ...
            style={{
              background: "linear-gradient(#0a0a0f, #0a0a0f) padding-box, linear-gradient(135deg, rgba(139,92,246,0.5), transparent, rgba(34,211,238,0.5)) border-box"
            }}
            animate={{ rotate: 360 }}
            transition={{ duration: 4, repeat: Infinity, ease: "linear" }}
          />
