...
const typeIcons: Record<string, React.ElementType> = {
  task: CheckCheck, message: MessageSquare, alert: AlertCircle,
  update: GitBranch, mention: User, system: Zap, default: Bell
};
...
export default function NotificationsPage() {
  ...
  const getType = (notification: any) => notification.type || notification.category || "default";
  ...
  const filteredNotifications = useMemo(() => {
    ...
    if (selectedType !== "all") result = result.filter(n => getType(n) === selectedType);
    if (searchQuery) {
  ...
  const groupedNotifications = useMemo(() => {
    ...
    filteredNotifications.forEach(notification => {
      ...
      else if (date > new Date(today.setDate(today.getDate() - 7))) key = "This Week";
      else key = "Earlier";
  ...
  const notificationTypes = ["all", "task", "message", "alert", "update", "mention", "system"];

  return (
    <SidebarProvider>
      <AppSidebar />
      <SidebarInset className={cn(
        isDark ? "bg-[#0a0a0f]" : "bg-neutral-50"
      )}>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className={cn(
                      ...
                      <DropdownMenuSeparator className={isDark ? "bg-white/[0.06]" : "bg-neutral-200"} />
                      <DropdownMenuItem className={cn(
                        "cursor-pointer",
                        isDark 
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className={cn(
                      ...
                      <span className="capitalize">{selectedType === "all" ? "All Types" : selectedType}</span>
                  ...
                  <DropdownMenuContent align="start" className={cn(
                    ...
                    {notificationTypes.map(type => {
                      const IconComponent = type === "all" ? Sparkles : (typeIcons[type] || Bell);
                      return (
                        <DropdownMenuItem
                          key={type}
                        ...
                        >
                          <IconComponent className={`w-4 h-4 mr-2 ${type !== "all" ? getTypeIconColor(type) : ""}`} />
                  ...
                  <DropdownMenuContent align="start" className={cn(
                    ...
                    <DropdownMenuItem 
                      ...
                      className={cn(
                        "cursor-pointer",
                        sortBy === "newest" 
                          ? isDark ? "text-white bg-white/[0.06]" : "text-neutral-900 bg-neutral-100"
                          : isDark ? "text-white/70 hover:text-white hover:bg-white/[0.04]" : "text-neutral-600 hover:text-neutral-900 hover:bg-neutral-50"
                      )}>
                      Newest First
                    </DropdownMenuItem>
                    <DropdownMenuItem 
              ...
              {filteredNotifications.length === 0 ? (
                ...
                >
                  ...
                  <p className={cn(
                    ...
                    {filter === "unread" ? "You've read all your notifications. Great job!" : "When you get notifications, they'll show up here."}
                  </p>
                </motion.div>
              ) : (
                <motion.div key="list" variants={containerVariants} initial="hidden" animate="visible" className="space-y-8">
                    ...
                    <div key={group}>
                      <motion.div variants={itemVariants} className="flex items-center gap-3 mb-4">
                        ...
                        <div className={cn(
                          ...
                          isDark ? "from-white/[0.08] to-transparent" : "from-neutral-200 to-transparent"
                        )} />
                        <span className={cn(
                          "text-xs",
                          isDark ? "text-white/30" : "text-neutral-400"
                        )}>{groupNotifications.length}</span>
                      </motion.div>
                            ...
                            return (
                              <motion.div
                                ...
                                className={cn(
                                  ...
                                  !notification.read 
                                    ? colorClass + (isDark ? " hover:border-emerald-500/40" : " hover:border-emerald-300")
                                    : isDark 
                              ...
                              >
                                        ...
                                        {notification.senderName && (
                                          <div className="flex items-center gap-2 mt-2">
                                            <div className={cn(
                                              ...
                                              )}>{getInitials(notification.senderName)}</span>
                                            </div>
                                            <span className={cn(
                                              "text-xs",
                                              isDark ? "text-white/40" : "text-neutral-500"
                                ...
                                <motion.div className={cn(
                                  ...
                                  isDark ? "from-emerald-500/5 to-cyan-500/5" : "from-emerald-500/5 to-cyan-500/5"
                                )} />
                              </motion.div>
                            );
                          })}
              ...
              >
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className={cn(
                    ...
                    )}>{notifications.length - unreadCount}</p>
                    <p className={cn(
                      "text-xs mt-1",
                      isDark ? "text-white/40" : "text-neutral-500"
                    )}>Read</p>
