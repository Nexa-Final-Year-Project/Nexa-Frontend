...
import { MenuIcon, PlusIcon, SearchIcon, UserIcon, Command, Sparkles } from "lucide-react";
    ...
    <SidebarProvider>
      ...
      <SidebarInset className={isDark ? "bg-[#0c0c10]" : "bg-[#fafafa]"}>
          ...
          <div className="flex items-center gap-2 px-3 md:px-6 justify-between w-full">
            ...
            <div className="flex items-center gap-3">
              <SidebarTrigger className={`
                ...
                group/trigger
              `}>
                <MenuIcon className={`w-4 h-4 ${isDark ? "text-white/60 group-hover/trigger:text-white/90" : "text-neutral-500 group-hover/trigger:text-neutral-700"} transition-colors`} />
              </SidebarTrigger>
              <Separator
                orientation="vertical"
                className={`h-6 ${isDark ? "bg-white/[0.06]" : "bg-neutral-200"} hidden sm:block`}
              />
            </div>
            ...
            <div className="flex items-center gap-3 px-2 md:px-8 flex-1 justify-center max-w-2xl">
              ...
              <div className="hidden lg:flex flex-1 max-w-lg relative group/search">
                <div 
                  ...
                  `}
                >
                  <SearchIcon className={`w-4 h-4 ${isDark ? "text-white/40" : "text-neutral-400"}`} />
                  <span className={`text-sm ${isDark ? "text-white/40" : "text-neutral-400"} flex-1`}>Search anything...</span>
                  <div className={`flex items-center gap-1.5 px-2 py-1 rounded-lg ${
                    isDark 
                      ? "bg-white/[0.04] border border-white/[0.06]" 
                      : "bg-white border border-neutral-200"
                  }`}>
                    <Command className={`w-3 h-3 ${isDark ? "text-white/40" : "text-neutral-400"}`} />
                    <span className={`text-[10px] ${isDark ? "text-white/40" : "text-neutral-400"} font-medium`}>K</span>
                  </div>
                </div>
              </div>
              ...
              <Button
                ...
                onClick={() => openModal("search.global")}
              >
                <SearchIcon className={`w-4 h-4 ${isDark ? "text-white/60" : "text-neutral-500"}`} />
              </Button>
              ...
              <Button
                ...
                size="sm"
              >
                <PlusIcon className={`w-4 h-4 ${isDark ? "text-neutral-900" : "text-white"} group-hover/create:rotate-90 transition-transform duration-300`} />
                <span className={`hidden md:inline ${isDark ? "text-neutral-900" : "text-white"} ml-2 text-sm font-medium`}>
                  Create
                </span>
              </Button>
            </div>
        ...
        `}>
          ...
          <div className={`absolute inset-0 ${isDark ? "opacity-[0.02]" : "opacity-[0.3]"} pointer-events-none`} style={{
            backgroundImage: `radial-gradient(circle at 1px 1px, ${isDark ? "white" : "#d4d4d4"} 1px, transparent 0)`,
